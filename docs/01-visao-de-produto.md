...
Desenhos são arquivos `.draw.md`: um markdown de verdade, com frontmatter, com o JSON da cena dentro. O ganho concreto é um só, e é grande: o texto escrito dentro de um desenho é pesquisável pela mesma busca das notas, e um `[[link]]` num elemento do canvas aparece nos backlinks da nota alvo, porque o link está literalmente no arquivo. Diff de Git legível e "abrir no Bloco de Notas" são consequência de o arquivo ser texto indentado, não do markdown.
...
- Desenhos `.draw.md` com formas, texto, conectores, post-its e imagens coladas — referenciados como `[[Nome.draw]]`.
