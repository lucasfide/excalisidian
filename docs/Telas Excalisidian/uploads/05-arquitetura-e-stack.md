...
├── canvas/
│   ├── EditorDesenho.tsx
│   ├── formatoDesenho.ts    parse e serialize do .excalidraw.md
│   ├── postit.ts
...
4. **O componente é não-controlado.** Você lê com `getSceneElements()`, `getAppState()`, `getFiles()` e escreve com `updateScene()`. O autosave é orquestrado por você, a partir do `onChange` com debounce.
   ...
   Detalhe da 0.18 que muda comportamento: `updateScene` trocou `commitToHistory: boolean` por `captureUpdate: CaptureUpdateAction`, e o default passou a ser `EVENTUALLY`. Duas consequências diretas — recarregar do disco na faixa de conflito precisa de `CaptureUpdateAction.NEVER`, senão a recarga entra no histórico de desfazer; e a inserção programática do post-it precisa de `IMMEDIATELY` para ser desfazível.
5. **`UIOptions.canvasActions`** permite desligar itens do menu (salvar, exportar, limpar, trocar tema). **Esconder ferramentas individuais da toolbar não tem API oficial** — é a issue #3012, aberta. A saída é CSS sobre classes internas, o que é frágil entre versões: **prenda a versão exata no `package.json`** e revalide a cada upgrade.
...
`formatoDesenho.ts` faz a ponte entre a cena do Excalidraw e o `.excalidraw.md`:
...
3. Resolução de link com nomes duplicados em pastas diferentes escolhe o esperado.
